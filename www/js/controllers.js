angular.module('app.controllers', [])
    ...
    $scope.loginEmail = function(formName,cred) {
      ...
      if(formName.$valid) {  // Check if the form data is valid or not
        ...
        firebase.auth().signInWithEmailAndPassword(cred.email,cred.password).then(function(result) {
          ...
          function(error) {
            sharedUtils.hideLoading();
            sharedUtils.showAlert("Please note","Erro de autenticação");
          }
        );

      }else{
        sharedUtils.showAlert("Please note","Dado inserido não é válido");
      }
  ...
  .controller('signupCtrl', function($scope,$rootScope,sharedUtils,$ionicSideMenuDelegate,
    ...
    $scope.signupEmail = function (formName, cred) {
      ...
      if (formName.$valid) {  // Check if the form data is valid or not
        ...
        sharedUtils.showAlert("Please note","O dado não inserido não é válido");
      }
  ...
  .controller('topicsCtrl', function($scope,$rootScope,$ionicSideMenuDelegate,
    ...
    $scope.itemManipulation = function(edit_val) {  // Takes care of item add and edit ie Item Manipulator
      ...
      if(edit_val!=null) {
        ...
        title="Editar Tópico";
        sub_title="Edite seu Tópico";
      }
      else {
        $scope.data = {};    // For adding new address
        title="Adicionar Tópico";
        sub_title="Adicione um novo tópico";
      }
      // An elaborate, custom popup
      var connectionPopup = $ionicPopup.show({
        template: '<input type="text"   placeholder="Tópico"  ng-model="data.topic"> <br/> ' +
                  '<input type="text"   placeholder="Informação" ng-model="data.info"> <br/> '+
                  '<input type="text"   placeholder="Imagem (Opicional)"  ng-model="data.img"> <br/> ',
        title: title,
        subTitle: sub_title,
        scope: $scope,
        buttons: [
    ...
    $scope.deleteTopic = function(del_id) {
      var confirmPopup = $ionicPopup.confirm({
        ...
        template: 'Você tem certeza que deseja deletar este tópico',
        buttons: [
          { text: 'Não' , type: 'button-stable' },
          { text: 'Sim', type: 'button-assertive' , onTap: function(){return del_id;} }
        ]
      });
  ...
  .controller('indexCtrl', function($scope,$rootScope,sharedUtils,$ionicHistory,$state,$ionicSideMenuDelegate) {
    ...
    $scope.logout=function(){
      ...
      firebase.auth().signOut().then(function() {
        ...
        sharedUtils.showAlert("Erro","Falha Logout")
      });
  ...
  .controller('graphCtrl', function($scope,$rootScope,fireBaseData,
    ...
    function doFail(e){
      sharedUtils.hideLoading();
      console.log("Error",e);
      sharedUtils.showAlert("Erro de configuração"," Verifique se a porta é para web-socket!");
  ...
  /*  var ISTOffset = 330;   // IST offset UTC +5:30
    ...
    $scope.notify=function(){
      sharedUtils.hideLoading();
      message = new Paho.MQTT.Message("0"); // -1 => Notify
      message.destinationName = mqttData.currentTopic;
      client.send(message);
    };
